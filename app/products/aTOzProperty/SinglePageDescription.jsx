...
const SinglePageDescription = ({ data, datas }) => {
  const [expandedIndex, setExpandedIndex] = useState(null);
  const [threeBotlteIcon, setThreeBotlteIcon] = useState(false);
  ...
  const firstThreeData = datas.slice(0, 3);

  return (
    <div className="bg-[#F5F5F7] py-8 md:py-16">
      <div className="container mx-auto p-5">
        {threeBotlteIcon && (
          <div className="text-[#343438]">
            <ThreeBottle /> {/* Render the passed icon */}
          </div>
        )}
        <Heading className="!text-left" title={data.mainHeading} />
